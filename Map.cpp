 ...
 */
bool Map::spawnHacker(int row, int col) {           //int room, Hackers hackerarray[20]
    // out of map bounds
...
bool Map::clearPosition(int row, int col) //this function works the same as the spawn, but instead clears hacker and npc locations
{
    // out of map bounds
    if (!(row >= 0 && row < num_rows && col >= 0 && col < num_cols)) {
        return false;
    ...
    // location must be blank to spawn
    if (mapData[row][col] != 'H' && mapData[row][col] != 'N') {
        return false;
    }
    else
    {
        ...
        {
            mapData[row][col] = '-';
            setHackerCount(hacker_count - 1); //if you beat the hacker, hackercount is decreased by 1
        }
        else if (mapData[row][col] == 'N')
        {
            mapData[row][col] = '-';
            setNPCCount(npc_count - 1);
        }
        return true;
    }
}